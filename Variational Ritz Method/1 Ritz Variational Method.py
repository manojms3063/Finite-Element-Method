...
for i in range(1, N+1):
    p_i = x**(i) - x**(i+1)
    F[i-1] = -float(sp.integrate(x**2 * p_i,(x, x0, x1)))
# print(F)
...
y = 0
for i in range(N):
    y += c[i]*(x**(i+1) - x**(i+2))  # Sum all basis functions with coefficients
...
# Exact solution
y_exact = ((sp.sin(x) + (2 * sp.sin(1 - x))) / sp.sin(1)) + (x**2 - 2)
...
# plt.savefig('sine_plot.png', dpi=700)
plt.show()